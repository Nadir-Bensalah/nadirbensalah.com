...
import { englishServicePages, findEnglishSeoPage } from '@/content/englishSeoPages';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const page = findEnglishSeoPage(englishServicePages, slug);
  if (!page) return {};
  return {
    title: page.title,
...
export default async function EnglishServicePage({ params }: PageProps) {
  ...
  const page = findEnglishSeoPage(englishServicePages, slug);
  if (!page) notFound();
  return <SeoLandingPage page={page} basePath="/en/services" />;
}
